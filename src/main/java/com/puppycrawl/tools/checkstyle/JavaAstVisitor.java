...
public final class JavaAstVisitor extends JavaParserBaseVisitor<DetailAstImpl> {
    ...
    public DetailAstImpl visitRecordComponent(JavaParser.RecordComponentContext ctx) {
        ...
        ctx.arrayDeclarator().forEach(arrayDeclaratorContext -> {
            type.addChild(visit(arrayDeclaratorContext));
        });
    ...
    public DetailAstImpl visitClassBlock(JavaParser.ClassBlockContext ctx) {
        ...
        // If block is static, we create imaginary STATIC_INIT node
        if (ctx.LITERAL_static() != null) {
            ...
            // Otherwise, we call it an INSTANCE_INIT
            final DetailAstImpl temp = classBlock;
    ...
    public DetailAstImpl visitClassOrInterfaceType(JavaParser.ClassOrInterfaceTypeContext ctx) {
        ...
        if (ctx.typeArguments() != null) {
            DetailAstPair.addASTChild(currentAST, visitTypeArguments(ctx.typeArguments()));
        }
    ...
    public DetailAstImpl visitAnnotations(JavaParser.AnnotationsContext ctx) {
        ...
        if (ctx.anno.isEmpty() && !ctx.createImaginaryNode) {
    ...
    public DetailAstImpl visitExpStat(JavaParser.ExpStatContext ctx) {
        ...
        // For some AST's, we attach the SEMI to the subtree.
        // Also, we get rid of 'EXPR' node
        if (expStatRoot.getType() == TokenTypes.CTOR_CALL
                || expStatRoot.getType() == TokenTypes.SUPER_CTOR_CALL) {
            expStatRoot.addChild(create(ctx.SEMI()));
        }
        else {
    ...
    @Override
    public DetailAstImpl visitCtorCall(JavaParser.CtorCallContext ctx) {
        final DetailAstImpl ctorCall = create(TokenTypes.CTOR_CALL,
                (Token) ctx.LITERAL_this().getPayload());
        ctorCall.addChild(create((Token) ctx.LPAREN().getPayload()));

        // We always add an 'ELIST' node
        final DetailAstImpl expressionList = Optional.ofNullable(visit(ctx.expressionList()))
                .orElse(createImaginary(TokenTypes.ELIST, ctx.RPAREN()));

        ctorCall.addChild(expressionList);
        ctorCall.addChild(create((Token) ctx.RPAREN().getPayload()));
        return ctorCall;
    }
    ...
    public DetailAstImpl visitMethodExp(JavaParser.MethodExpContext ctx) {
        ...
        dot.addChild(create(ctx.mCall.start));
        return root;
    }

    @Override
    public DetailAstImpl visitSuperCall(JavaParser.SuperCallContext ctx) {
        final DetailAstImpl superCall = create(TokenTypes.SUPER_CTOR_CALL,
                (Token) ctx.LITERAL_super().getPayload());
        superCall.addChild(create(ctx.LPAREN()));

        // We always add an 'ELIST' node
        final DetailAstImpl expressionList = Optional.ofNullable(visit(ctx.expressionList()))
                .orElse(createImaginary(TokenTypes.ELIST, ctx.RPAREN()));

        superCall.addChild(expressionList);
        superCall.addChild(create(ctx.RPAREN()));
        return superCall;
    }
    ...
    @Override
    public DetailAstImpl visitParamPrimary(JavaParser.ParamPrimaryContext ctx) {
        return flattenedTree(ctx);
    }
    ...
    public DetailAstImpl visitSuperExp(JavaParser.SuperExpContext ctx) {
        ...
        if (superSuffixParent.getFirstChild() != null
            && superSuffixParent.getFirstChild().getFirstChild() != null) {
            superSuffixParent.getFirstChild()
                    .getFirstChild().addPreviousSibling(bop);
        }
    ...
    public DetailAstImpl visitSimpleMethodExp(JavaParser.SimpleMethodExpContext ctx) {
        ...
        // Add IDENT if applicable
        if (!TokenUtil.isOfType(root, TokenTypes.CTOR_CALL, TokenTypes.SUPER_CTOR_CALL)) {
    ...
    @Override
    public DetailAstImpl visitMethodReference(JavaParser.MethodReferenceContext ctx) {
        final DetailAstImpl doubleColon = create(TokenTypes.METHOD_REF,
                (Token) ctx.DOUBLE_COLON().getPayload());
        final List<ParseTree> children = ctx.children.stream()
                .filter(child -> !child.equals(ctx.DOUBLE_COLON()))
    ...
    @Override
    public DetailAstImpl visitClassType(JavaParser.ClassTypeContext ctx) {
        return flattenedTree(ctx);
    }
    ...
    @Override
    public DetailAstImpl visitNonWildcardDiamond(JavaParser.NonWildcardDiamondContext ctx) {
        final DetailAstImpl typeArguments =
                createImaginary(TokenTypes.TYPE_ARGUMENTS, ctx.LT());
        typeArguments.addChild(create(TokenTypes.GENERIC_START,
                (Token) ctx.LT().getPayload()));
    ...
    @Override
    public DetailAstImpl visitNonWildcardTypeArgs(JavaParser.NonWildcardTypeArgsContext ctx) {
        return flattenedTree(ctx);
    }
    ...
    public DetailAstImpl visitTypeArgumentsTypeList(JavaParser.TypeArgumentsTypeListContext ctx) {
        ...
        for (int i = 0; i < ctx.COMMA().size(); i++) {
            ...
            final DetailAstImpl ident = visit(ctx.typeType(0));
    ...
    @Override
    public DetailAstImpl visitSuperSuffix(JavaParser.SuperSuffixContext ctx) {
        final DetailAstImpl root;
        // If we have a method call, lparen becomes parent
        if (ctx.LPAREN() != null) {
            root = create(TokenTypes.METHOD_CALL,
                    (Token) ctx.LPAREN().getPayload());
        }
        else {
            root = create(ctx.DOT());
    ...
    private DetailAstImpl buildRootTree(DetailAstImpl innerChild, ParserRuleContext ctx) {
        ...
        for (int i = 1; ctx.children != null && i < ctx.children.size() - 1; i += 2) {
